...
def setup_model(opt):
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, opt.lr_drop, gamma=opt.gamma_lr_drop)
    ...
    if opt.resume is not None:
        ...
        logger.info(f"Loaded model saved at epoch {checkpoint['epoch']+1} from checkpoint: {opt.resume}")
