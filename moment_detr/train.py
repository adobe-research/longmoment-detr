...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    if opt.start_epoch is None:
        start_epoch = -1 if opt.is_eval else 0
    else:
        start_epoch = opt.start_epoch
    save_submission_filename = "latest_{}_{}_preds.jsonl".format(opt.dset_name, opt.val_split_name)
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        if epoch_i > -1:
            train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer)
            lr_scheduler.step()
        eval_epoch_interval = 1
        if opt.val_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            with torch.no_grad():
            ...
            stop_score = metrics["brief"]["MR-full-R1@0.5"] + metrics["brief"]["MR-full-R1@0.7"]
...
def start_training():
    ...
    if opt.val_path is not None:
        dataset_config["data_path"] = opt.val_path
    ...
    return opt.ckpt_filepath.replace(".ckpt", "_best.ckpt"), opt.val_split_name, opt.val_path, opt.debug, opt.eval_split_name, opt.eval_path


if __name__ == '__main__':
    best_ckpt_path, val_split_name, val_path, debug, eval_split_name, eval_path = start_training()
    if not debug:
